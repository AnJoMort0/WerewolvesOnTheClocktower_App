...
export function FoxRevealModal({ session, language, pending = false, connected = true, onConfirm, onClose, onReopen }: {
  session: PhoneView;
  language: Language;
  pending?: boolean;
  connected?: boolean;
  ...
  return <>
    ...
    <GameModal open={session.visible !== false} onClose={onClose} title={getRoleLabel("v04", language)}
      ...
      <div className="space-y-4">
        <PhoneActionScreen session={session} playerId="fox-selection" language={language}
          pending={pending} connected={connected} selectedPlayerId={targetId} showHeader={false}
