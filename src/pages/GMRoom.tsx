...
function getIndependentActionUses(powerState: ActorPowerState | undefined, role: RoleId): number | undefined {
  if (!powerState) return undefined;
...
const GMRoom = () => {
  ...
  const getStoredDogWolfFallbackState = useCallback((playerId: string) => {
    ...
    const parsed = parsePlayerCharacter(player.character);
    const metadata = parsePlayerCharacterMetadata(player.character);
    if (!metadata.ownerPlayerId) return null;
    if (parsed.baseRole !== "a02" && parsed.actorCopiedRole !== "a02") return null;
    return createDogWolfState(metadata.ownerPlayerId);
  }, [players]);
  ...
  const v23UsesByPlayerId = useMemo(() => {
    ...
    for (const [playerId, role] of Object.entries(abilityRoleAssignments)) {
      ...
      if (spiderWebbedDiedSourcePlayerIds.includes(playerId)) {
        uses[playerId] = 0;
        continue;
      }
    ...
    return uses;
  }, [abilityRoleAssignments, independentPowerStates, mimeMechanicalRole, mimePlayerId, roleAssignments, spiderDayChangeUsed, spiderWebbedDiedSourcePlayerIds]);
  ...
  const handleIndependentPowerStateChange = useCallback((playerId: string, next: ActorPowerState) => {
    ...
    } else if (storedDogFallbackState) {
      setDogWolfStates((previous) => ({
        ...previous,
        [playerId]: { ...storedDogFallbackState, powerState: next },
      }));
  ...
  const changeRole = (playerId: string, role: RoleId) => {
    ...
    if (oldRole === "a03" && role !== "a03") {
      ...
      setMimeRevealCards([]);
      setMimeDayImmunityTargetIds([]);
      setPoisonTargetsBySource((prev) => {
        if (!mimeWitchPoison || prev[playerId] !== mimeWitchPoison.targetPlayerId) return prev;
        const next = { ...prev };
  ...
  const clearDueMimeWitchPoison = useCallback((progressOrder: number) => {
    if (!mimePlayerId || !mimeWitchPoison) return;
    if (mimeWitchPoison.nightNumber >= nightNumber) return;
    if (getScriptOrderIndex("a03") > progressOrder) return;
    setPoisonTargetsBySource((previous) => {
      if (previous[mimePlayerId] !== mimeWitchPoison.targetPlayerId) return previous;
      const next = { ...previous };
      delete next[mimePlayerId];
      return next;
    });
    setMimeWitchPoison(null);
  }, [mimePlayerId, mimeWitchPoison, nightNumber]);

  const clearEffectsFromDeadSources = useCallback((progressOrder: number) => {
    setPoisonTargetsBySource((previous) => Object.fromEntries(
      Object.entries(previous).filter(([sourcePlayerId, targetPlayerId]) => {
        if (sourcePlayerId === "manual") return true;
        const isMimeWitchPoison = sourcePlayerId === mimePlayerId
          && mimeWitchPoison?.targetPlayerId === targetPlayerId;
        if (isMimeWitchPoison && mimeWitchPoison.nightNumber >= nightNumber) return true;
        const ownerPlayerId = dogWolfStates[sourcePlayerId]?.ownerPlayerId;
        const sourceInactive = permanentlyDead.has(sourcePlayerId)
          || (!!ownerPlayerId && permanentlyDead.has(ownerPlayerId));
        const sourceRole = isMimeWitchPoison
          ? "a03"
          : ownerPlayerId
          ? effectiveRoleAssignments[ownerPlayerId]
          : abilityRoleAssignments[sourcePlayerId];
    ...
    if (deadSourceRoles.includes("e02")) {
      setPoisonTargetsBySource((previous) => Object.fromEntries(
        Object.entries(previous).filter(([sourcePlayerId, targetPlayerId]) => {
          if (sourcePlayerId === "manual") return true;
          if (sourcePlayerId === mimePlayerId && mimeWitchPoison?.targetPlayerId === targetPlayerId) return true;
  ...
  const handleScriptLineCompleted = useCallback((key: string, completed: boolean, progressOrder: number | null = null, participantIds: string[] = []) => {
    ...
    if (completed && progressOrder !== null) {
      clearDueMimeWitchPoison(progressOrder);
  ...
  const handleDragAction = useCallback((
    ...
    applyCaughtIfWebbed();
    if (action === "poison") {
      const dueMimeWitchPoison = sourceIsMimeCopying("e02")
        && sourcePlayerId
        && !!mimeWitchPoison
        && mimeWitchPoison.nightNumber < nightNumber;
      const previousMimeWitchTarget = sourceIsMimeCopying("e02") && sourcePlayerId && !dueMimeWitchPoison
        ? poisonTargetsBySource[sourcePlayerId]
        : null;
      if (dueMimeWitchPoison && sourcePlayerId) {
        setPoisonTargetsBySource((previous) => {
          if (previous[sourcePlayerId] !== mimeWitchPoison.targetPlayerId) return previous;
          const next = { ...previous };
          delete next[sourcePlayerId];
          return next;
        });
      ...
      if (roleSource === "a02") {
      ...
      else if (roleSource === "v12") {
        const poisonStealerId = sourcePlayerId && abilityRoleAssignments[sourcePlayerId] === "v12"
          ? sourcePlayerId
          : getRolePlayerId("v12");
        if (!poisonStealerId) return;
        const poisonSourceEntries = Object.entries(poisonTargetsBySource)
          .filter(([, poisonedPlayerId]) => poisonedPlayerId === targetPlayerId);
        if (poisonSourceEntries.length === 0) {
          toast.warning(getToast("warnNoTargets", (room?.language as Language) || "pt"));
          return;
        }
        ...
        setPoisonTargetsBySource((previous) => {
          let changed = false;
          const next = { ...previous };
          for (const [poisonSourcePlayerId, poisonedPlayerId] of Object.entries(previous)) {
            if (poisonedPlayerId !== targetPlayerId) continue;
            next[poisonSourcePlayerId] = poisonStealerId;
            changed = true;
            if (poisonSourcePlayerId !== "manual") {
              pendingGameActionLogSourcesRef.current.set(`poison:${poisonStealerId}`, poisonSourcePlayerId);
            }
          }
          return changed ? next : previous;
        });
        if (mimeWitchPoison && poisonSourceEntries.some(([poisonSourcePlayerId]) => poisonSourcePlayerId === mimePlayerId)) {
          setMimeWitchPoison((previous) => previous?.targetPlayerId === targetPlayerId
            ? { ...previous, targetPlayerId: poisonStealerId }
            : previous);
        }
        setNightTargetedPlayerIds((prev) => { const n = new Set(prev); n.add(targetPlayerId); return n; });
  ...
  const applyPhoneAction = useCallback(({ action, targetPlayerId, targetPlayerIds, sourcePlayerId, monkeyReveal, foxReveal }: PhoneAction) => {
    ...
    if (action === PHONE_MODE.FOX_TAMER_CHECK) {
      ...
      if (foxReveal.foxRanAway) {
        const powerState = independentPowerStates[sourcePlayerId];
        if (powerState) handleIndependentPowerStateChange(sourcePlayerId, { ...powerState, foxDisabled: true });
  ...
  const resolvePlayerActionRequest = useCallback(async (request: PlayerActionRequest, accepted: boolean) => {
    ...
    const preservesSpiderFreeWebChange = request.kind === "v23-web"
      && spiderWebbedDiedSourcePlayerIds.includes(request.actorPlayerId);
    const independentActionUses = getIndependentActionUses(
      independentPowerStates[request.actorPlayerId] ?? getStoredDogWolfFallbackState(request.actorPlayerId)?.powerState,
      actionRole,
    );
    const hasIndependentActionState = independentActionUses !== undefined;
    const shouldSpendUse = shouldApply
      ...
      && (requestUsesByPlayerId[request.actorPlayerId] !== undefined || hasIndependentActionState);

    let nextState = removePlayerActionRequest(pruneResolvedPlayerActionState(playerActionState), request.id);
    const currentUses = requestUsesByPlayerId[request.actorPlayerId]
      ?? independentActionUses
      ?? nextState.powerUses[actionRole]?.[request.actorPlayerId]
      ?? 0;
  ...
  const getMimeCopyCandidates = useCallback((mimeId: string): MimeCopyCandidate[] => {
    ...
    const dogOwnerRoles = activeDogWolfPlayerIds.flatMap((dogPlayerId) => {
      ...
      return ownerRole && ownerRole !== "a02" && MIME_COPY_ROLES.includes(ownerRole) ? [ownerRole] : [];
    });
    const drunkardExcludedRoles = Object.entries(roleAssignments)
      .filter(([playerId, role]) => playerId !== mimeId && role !== "a01")
      .map(([, role]) => role);
    const drunkardReplacementCandidates = getDrunkardReplacementCandidates(drunkardExcludedRoles)
      .filter((role): role is RoleId => MIME_COPY_ROLES.includes(role));
    const drunkardFallbackRole = drunkardReplacementRole
      ? MIME_COPY_ROLES.includes(drunkardReplacementRole) ? drunkardReplacementRole : null
    ...
    return Object.entries(roleAssignments).flatMap(([playerId, role]) => {
      ...
      if (!MIME_COPY_ROLES.includes(role)) return [];
      ...
      if (role === "s01" && !hasAliveLover) return [];
      if (role === "a01") {
  ...
  const handleMimeReveal = useCallback((sourcePlayerId?: string | null) => {
    ...
    if (gmSnapshotLoaded && room?.status === "playing") {
      const mimeSnapshot = getPlayerLogSnapshot(mimeId);
      if (mimeSnapshot) {
        const copiedLabel = pick.displayRole === pick.mechanicalRole
          ? getRoleLabel(pick.displayRole, localLang)
          : `${getRoleLabel(pick.displayRole, localLang)} (${getRoleLabel(pick.mechanicalRole, localLang)})`;
        recordGameEvent({
          action: "role_change",
          actor: mimeSnapshot,
          actorRole: "a03",
          target: { ...mimeSnapshot, role: pick.mechanicalRole },
          detail: `${getRoleLabel("a03", localLang)} -> ${copiedLabel}`,
        });
      }
    }
  ...
  }, [getMimeCopyCandidates, getPlayerLogSnapshot, gmSnapshotLoaded, markScriptRoleAction, mimePlayerId, permanentlyDead, recordGameEvent, roleAssignments, room?.language, room?.status, roomId]);
  ...
  const buildLittleGirlCards = useCallback((viewerPlayerId: string | null): RevealCard[] => {
    ...
    return redX.map((pid) => {
      const killerPlayerId = killSourcePlayerIds[pid];
      const card = resolveKillerCard(
        killSources[pid],
        roleAssignments,
        killerPlayerId && illusionPlayerIds.has(killerPlayerId) ? killerPlayerId : null,
        localLang,
      );
      const name = players.find((p) => p.id === pid)?.name;
      return { name, image: card.image, label: card.label, roleId: card.roleId };
    });
  }, [isPlayerActingPoisoned, room, roleAssignments, playerStatuses, killSources, killSourcePlayerIds, illusionPlayerIds, players]);
  ...
  const handleLamplighterReveal = useCallback((sourcePlayerId?: string | null) => {
    ...
    const byPlayerId = Object.fromEntries(recipients.map((viewerPlayerId) => {
      ...
      } else if (illusionPlayerIds.has(pick.id)) {
        role = "a06";
        charges = [];
      }
  ...
  const handleWerewolfSeerReveal = useCallback((sourcePlayerId?: string | null) => {
    ...
    const byPlayerId = Object.fromEntries(recipients.map((viewerPlayerId) => {
      ...
      } else if (illusionPlayerIds.has(werewolfSeerVictim.id)) role = "a06";
  ...
  }, [getRevealRecipientIds, handleIndependentPowerStateChange, handlePlayerStatusChange, illusionPlayerIds, independentPowerStates, isPlayerActingPoisoned, werewolfSeerVictim, markScriptRoleAction, roleAssignments, roomId]);
  ...
  const deathTriggeredSourcePlayerIds = useMemo(() => ({
    ...
    spiderWebbedDied: spiderWebbedDiedSourcePlayerIds,
  }), [colossusAttackedPlayerIds, abilityRoleAssignments, actorPlayerId, effectiveActorCopiedRole, lastNightDeadPlayerIds, playerEffects, roleAssignments, spiderWebbedDiedSourcePlayerIds]);
  ...
  const conditionKeys = useMemo(() => {
    ...
    // Cupid has charges left and at least one living lover to protect
    keys["cupidHasCharges"] = hasAliveLover && (
      mimeMechanicalRole === "s01"
      || cupidCharges < 2
      || Object.entries(independentPowerStates)
                ...
                <div className="space-y-2">
                  ...
                  {players
                    ...
                    .map((player) => {
                      ...
                      const defaultObjectiveEffects: StatusEffect[] = objectiveRoleId
                        ? WEREWOLF_ROLES.includes(objectiveRoleId)
                          ? ["werewolf_turned"]
                          : EVIL_ROLES.includes(objectiveRoleId)
                          ? ["evil_being"]
                          : []
                        : [];
                      const visibleEffectIcons = [
                        ...defaultObjectiveEffects,
                        ...Array.from(effects).filter((effect) => !defaultObjectiveEffects.includes(effect)),
                      ];
                      const hasHostileObjectiveMarker = defaultObjectiveEffects.length > 0 || isWerewolfTurned || isEvilBeing;
                      ...
                      const rowContent = (
                        ...
                        >
                          ...
                          {/* Status effect icons */}
                          {visibleEffectIcons.length > 0 && (
                            <div className="flex gap-0.5 flex-shrink-0">
                              {visibleEffectIcons.map(eff => STATUS_EFFECT_ICONS[eff] ? (
